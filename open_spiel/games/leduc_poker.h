...
class LeducState : public State {
 ...
 private:
  ...
  int NumCalls(std::vector<int>& sequence);
  int NumRaises(std::vector<int>& sequence);
  std::pair<std::pair<std::vector<int>,std::vector<int>>, std::pair<std::vector<int>,std::vector<int>>> SplitSequence();
  std::pair<bool,bool> FoldBeforePublic();
