...
int LeducState::NumCalls(std::vector<int>& sequence){
  int result = 0;
  for(int elem : sequence){
    if(elem == ActionType::kCall){
      result++;
    }
  }
  return result;
...
int LeducState::NumRaises(std::vector<int>& sequence){
  int result = 0;
  for(int elem : sequence){
    if(elem == ActionType::kRaise){
      result++;
    }
  }
  return result;
...
std::pair<std::pair<std::vector<int>,std::vector<int>>, std::pair<std::vector<int>,std::vector<int>>> LeducState::SplitSequence() {
  std::vector<int> p0_s1;
  std::vector<int> p0_s2;
  std::vector<int> p1_s1;
  std::vector<int> p1_s2;

  for(int i = 0; i < round1_sequence_.size(); i++){
    if(i % 2 == 0){
      p0_s1.push_back(round1_sequence_[i]);
    }
    else {
      p1_s1.push_back(round1_sequence_[i]);
    }
  }

  for(int i = 0; i < round2_sequence_.size(); i++){
    if(i % 2 == 0){
      p0_s2.push_back(round2_sequence_[i]);
    }
    else{
      p1_s2.push_back(round2_sequence_[i]);
    }
  }

  std::pair<std::vector<int>,std::vector<int>> s1 = std::make_pair(p0_s1,p1_s1);
  std::pair<std::vector<int>,std::vector<int>> s2 = std::make_pair(p0_s2,p1_s2);
  return std::make_pair(s1,s2);
}

std::pair<bool, bool> LeducState::FoldBeforePublic() {
  int i = 0;
  while(i < round1_sequence_.size()){
    if(round1_sequence_[i] == ActionType::kFold) {
      if(i % 2 == 0){
...
std::string LeducState::GetSaveString() {
  std::pair<bool,bool> fold = FoldBeforePublic();
  std::pair<std::pair<std::vector<int>,std::vector<int>>, std::pair<std::vector<int>,std::vector<int>>> s_Sequence = SplitSequence();
  std::vector<int> p0_s1 = s_Sequence.first.first;
  std::vector<int> p1_s1 = s_Sequence.first.second;
  std::vector<int> p0_s2 = s_Sequence.second.first;
  std::vector<int> p1_s2 = s_Sequence.second.second;
  int p0_s1_raises = NumRaises(p0_s1);
  int p0_s1_calls = NumCalls(p0_s1);
  int p0_s2_raises = NumRaises(p0_s2);
  int p1_s1_raises = NumRaises(p1_s1);
  int p1_s1_calls = NumCalls(p1_s1);
  int p1_s2_raises = NumRaises(p1_s2);
  std::string p0_result;
  std::string p1_result;

  if(private_cards_[0] == 0 || private_cards_[0] == 1){
    if(fold.first && p0_s1_calls == 0 && p0_s1_raises == 0) {
      p0_result = "P0 : bad hand : folded\n";
    } else {
      p0_result = "P0 : bad hand : played\n";
    }
  }

  if(private_cards_[1] == 0 || private_cards_[1] == 1){
    if(fold.second && p1_s1_calls == 0 && p1_s1_raises == 0) {
      p1_result = "P1 : bad hand : folded\n";
    } else {
      p1_result = "P1 : bad hand : played\n";      
    }
  }

  if(private_cards_[0] == 2 || private_cards_[0] == 3){
    if(p0_s1_raises > 0 || (p1_s1_raises > 0 && !fold.first)){
      p0_result = "P0 : semibluff : played\n";
    } else {
      p0_result = "P0 : semibluff : missed\n";
    }
  }

  if(private_cards_[1] == 2 || private_cards_[1] == 3){
    if(p1_s1_raises > 0 || (p0_s1_raises > 0 && !fold.second)){
      p1_result = "P1 : semibluff : played\n";
    } else {
      p1_result = "P1 : semibluff : missed\n";
    }
  }

  if(private_cards_[0] == 4 || private_cards_[0] == 5){
    if(fold.second){
      if(p0_s1_raises > 0){
        p0_result = "P0 : fastplay : played\n";
      } else {
        p0_result = "P0 : fastplay : missed\n";
      }
    } else {
      if (p0_s1_raises > 0 && p0_s2_raises > 0){
        p0_result = "P0 : fastplay : played\n";
      } else {
        p0_result = "P0 : fastplay : missed\n";
      }
  ...
  if(private_cards_[1] == 4 || private_cards_[1] == 5){
    if(fold.first){
      if(p1_s1_raises > 0){
        p1_result = "P1 : fastplay : played\n";
      } else {
        p1_result = "P1 : fastplay : missed\n";
      }
    } else {
      if (p1_s1_raises > 0 && p1_s2_raises > 0){
        p1_result = "P1 : fastplay : played\n";
      } else {
        p1_result = "P1 : fastplay : missed\n";
      }
  ...
  else if (winner_[1])
  {
    ...
    result = "ERROR: Generating log only possible for 2 players!\n";
    return result;
  }

  result.append(p0_result);
  result.append(p1_result);
  
  return result;
}

void LeducState::WriteGameToFile(){
  std::fstream fs;
